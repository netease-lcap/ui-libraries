...
namespace nasl.ui {
  @IDEExtraInfo({
    order: 5,
    sourceDocURL: 'https://tdesign.tencent.com/vue/components/date-picker',
    ideusage: {
      idetype: 'element',
    },
  })
  ...
  @IDEExtraInfo({
    ...
    extends: [{
      ...
      name: 'ElDateTimePickerPro',
    }],
  })
  @Component({
    title: '表单日期时间选择器',
    description: '表单日期时间选择器',
    group: 'Form',
  })
  export class ElFormDateTimePickerPro extends ViewComponent {
    constructor(options?: Partial<ElFormDateTimePickerProOptions & ElFormItemProOptions & Omit<ElDateTimePickerProOptions, keyof ElFormItemProOptions>>) {
      super();
    }
  }

  export class ElFormDateTimePickerProOptions extends ViewComponentOptions {
    @Prop<ElFormDateTimePickerProOptions, 'useRangeValue'>({
      group: '数据属性',
      title: '使用区间字段',
    ...
    @Prop<ElFormDateTimePickerProOptions, 'range'>({
      group: '数据属性',
      title: '区间选择',
      description: '是否支持进行时间区间选择，关闭则为时间点选择',
      setter: {
