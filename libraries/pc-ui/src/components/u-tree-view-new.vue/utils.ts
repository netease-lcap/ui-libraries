...
function forEachTreeData<T>(data: T[], { childrenField, parent }: { childrenField: string; parent?: T }, callback: (item: T, parent: T | undefined) => void) {
  data.forEach((item) => {
...
export function addChildrenValues<T, V>(item: T, checkList: Set<V>, { childrenField, valueField, disabledField }: { childrenField: string, valueField: string; disabledField: string }) {
  const children = _get(item, childrenField);
...
export function removeChildrenValues<T, V>(item: T, checkList: Set<V>, { childrenField, valueField, disabledField }: { childrenField: string, valueField: string; disabledField: string }) {
  const children = _get(item, childrenField);
...
export function addParentValues<T, V>(parent: T | undefined, { dataMap, childrenField, valueField, disabledField }: { dataMap: Record<any, undefined | { item: T, parent: T}>; childrenField: string; valueField: string; disabledField: string }, checkList: Set<V>, halfCheckList: Set<V>) {
  if (!parent) {
    return;
  }
  ...
  if (checked) {
    checkList.add(val);
    addParentValues(pos, { dataMap, childrenField, valueField, disabledField }, checkList, halfCheckList);
    return;
  }
...
export function filterParentValues<T, V extends string | number>(data: T[], values: V[],  { valueField, childrenField, disabledField }) {
...
export function isDisabled<T, V extends string | number>(data: T[], v: V, {  valueField, childrenField, disabledField }) {
  let disabled = false;
  const dataMap: Record<V, undefined | { item: T, parent: T}> = getDataMap(data, { valueField, childrenField });
...
export function getCheckInfo<T, V extends string | number>(data: T[], values: V[], { valueField, childrenField, disabledField }) {
  ...
  values.forEach((v) => {
    ...
    addParentValues(parent, { dataMap, childrenField, valueField, disabledField }, checkList, halfCheckList);
  });
