...
function getTouch(el: any, x: number, y: number) {
  return {
    identifier: Date.now(),
    target: el,
    pageX: x,
...
export function trigger(wrapper: Wrapper<Vue> | HTMLElement | Window, eventName: string, x = 0, y = 0, options: any = {}) {
...
export function triggerDrag(el: Wrapper<Vue> | HTMLElement, relativeX = 0, relativeY = 0): void {
  let x = relativeX;
  let y = relativeY;
  let startX = 0;
  let startY = 0;
