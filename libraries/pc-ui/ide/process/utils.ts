...
export function genViewVariables(
  ...
  return variableConfigList
    .map((variableConfig) => {
      const { varName, type, isMainEntity } = variableConfig;
      // case1: 主实体
      if (isMainEntity) {
        ...
        // 子表单删除信息局部变量
        const deleteInfoVarName = `del${firstUpperCase(varName)}`;
        return isApprovePage
          ? `let ${deleteInfoVarName}: ${type};`
...
export function genProcessV2LaunchLogic(
  ...
  return `export function ${nameGroup.processLaunch}(${params} ) {
    ...
    let result;
    ${dataCreateStr}
    result = nasl.processV2.launchProcess(nasl.util.NewAnonymousStructure({ ${dataArgs} }), '${process.uniqueKey}');
    return result;
  }`;
}
