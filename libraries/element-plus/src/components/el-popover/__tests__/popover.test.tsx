...
import { usePopperContainerId, useZIndex, rAF, ElPopperTrigger } from '@ep-test/test-utils';
...
const _mount = (props?: Partial<PopoverProps>) => mount(
    {
      setup() {
        const slots = {
          default: () => AXIOM,
...
describe('Popover.vue', () => {
  ...
  //   expect(Number.parseInt(window.getComputedStyle(findContentComp().element).zIndex)).toBeLessThanOrEqual(
  //     currentZIndex.value,
  //   );
  // });
  ...
  //   vi.useRealTimers();
  //   await rAF();
  //   expect(wrapper.findComponent(Popover).emitted()).toHaveProperty('after-enter');
  ...
  //   vi.useRealTimers();
  //   await rAF();
  //   expect(wrapper.findComponent(Popover).emitted()).toHaveProperty('after-leave');
  // });
