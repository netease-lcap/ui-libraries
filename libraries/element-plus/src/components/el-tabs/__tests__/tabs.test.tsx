...
import type { TabsPaneContext } from 'element-plus/es/components/tabs';
// import Tabs from '../src/tabs';
// import TabPane from '../src/tab-pane.vue';
import TabNav from 'element-plus/es/components/tabs/src/tab-nav';
import { ElTabPane as TabPane, ElTabs as Tabs } from '../index';
...
describe('Tabs.vue', () => {
  ...
  test('editable', async () => {
    ...
    const wrapper = mount(() => (
      <Tabs ref="tabs" v-model={editableTabsValue.value} type="card" editable onEdit={handleTabsEdit}>
    ...
    const wrapper = mount(() => (
      ...
      >
        {editableTabs.value.map((item) => (
          <TabPane label={item.title} key={item.name} name={item.name} />
        ))}
      </Tabs>
    ));
  ...
  test('keyboard event', async () => {
    ...
    await wrapper.find('#tab-second').trigger('keydown', { code: EVENT_CODE.left });
    expect(activeName.value).toEqual('fourth');
  });
  ...
  test('DOM update finished calculating navOffset', async () => {
    ...
    const mockOffsetLeft = vi
      .spyOn(wrapper.find('#tab-99').element as HTMLElement, 'offsetLeft', 'get')
    ...
    expect(tabsWrapper.find('.el-tabs__active-bar').attributes().style).toMatch('translateX(100px)');
  ...
  test('set modelValue asynchronously using beforeLeave', async () => {
    ...
    const wrapper = mount(() => (
      <Tabs v-model={activeName.value} onTabClick={handleTabClick} beforeLeave={beforeLeave}>
        <TabPane name="tab1" label="tab1">
          Tab 1 content
        </TabPane>
        <TabPane name="tab2" label="tab2">
