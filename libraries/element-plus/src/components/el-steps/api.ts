...
namespace nasl.ui {
  @IDEExtraInfo({
    ...
    ideusage: {
      ...
      dataSource: {
        // 当datasource没绑定，且没有子元素时，不设置dataSource属性
        dismiss:
          "{return !this.getAttribute('dataSource')?.expression && this.getDefaultElements().length > 0}",
        display: 3,
        loopRule: 'nth-child(n+2)',
        loopElem: ".el-step",
        emptySlot: {
          display: 'large',
          condition: "!this.getAttribute('dataSource')?.expression",
          accept: false,
        },
      },
    },
