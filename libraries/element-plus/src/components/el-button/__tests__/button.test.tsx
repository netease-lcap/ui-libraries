...
import Button, { ElButtonGroup as ButtonGroup } from '../index.ts';
...
describe('Button.vue', () => {
    ...
    it('loading slot', () => {
      const wrapper = mount({
        setup: () => () => (
          <Button v-slots={{ loading: () => <span class="custom-loading">111</span> }} loading>
            Loading
          </Button>
          ),
      });
  ...
  describe('Button Group', () => {
    ...
    it('use custom tag disabled click not triggered', async () => {
      ...
      const wrapper = mount(() => (
        <div>
          <Button tag="div" loading={isLoaing.value} disabled={isDisabled.value} />
        </div>
      ));
