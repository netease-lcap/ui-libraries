...
import { ElTreePlus as Tree } from '../index';
...
describe('Tree.vue', () => {
  ...
  test('checkOnNodeClick', async () => {
    ...
    await firstNodeContentWrapper.trigger('click');
    expect((treeWrapper.vm as InstanceType<typeof Tree>).getCheckedKeys()).toEqual([1, 11, 111]);
  });
  ...
  test('update tree-data after current-node-key', async () => {
    ...
    vm.data = [..._data];
    await nextTick();
    const currentNodeLabelWrapper2 = wrapper.find('.is-current .el-tree-node__label');
    ...
    const { wrapper } = getTreeVm(':props="defaultProps" :render-content="renderContent"', {
      methods: {
        renderContent(h, node) {
          return h('div', { class: 'custom-content' }, [h('button', { class: 'el-button' }, [node.node.label])]);
        },
      },
    });
  ...
  test('custom-node-class', async () => {
    ...
    const currentNodeLabelWrapper = wrapper.find('.is-test .el-tree-node__label');
  ...
  test('navigate with defaultExpandAll', () => {
    ...
    expect(
      Object.values((tree.vm as InstanceType<typeof Tree>).store.nodesMap).filter((item) => item.canFocus).length,
    ).toBe(9);
  });
  ...
  test('custom children and label', async () => {
    ...
    const firstChildrenNode = firstNode.element.querySelector('.el-tree-node__children');
  ...
  test('navigate with lazy and without node-key', async () => {
    ...
    await nextTick();
    await nextTick();
    expect(Object.values((tree.vm as InstanceType<typeof Tree>).store.nodesMap)).toHaveLength(5); // The root node (void node) + 4 child nodes (region1, region2, zone1, zone2)
    expect(
      Object.values(
        Object.values((tree.vm as InstanceType<typeof Tree>).store.nodesMap).filter((item) => item.canFocus).length
          === 4,
  ...
  test('customize some node contents', async () => {
    ...
    const nodeLabelWrapper2 = nodeContentWrapper2.find('span.el-tree-node__label');
