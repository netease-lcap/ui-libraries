...
import { ElProgress as Progress } from '../index';
...
describe('Progress.vue', () => {
  ...
  test('color', () => {
    ...
    expect(wrapper.find('.el-progress-bar__inner').attributes('style')).toContain(
  ...
  test('striped flow', () => {
    ...
    expect(wrapper.find('.el-progress-bar__inner--striped').exists()).toBe(true);
    expect(wrapper.find('.el-progress-bar__inner--striped-flow').exists()).toBe(true);
  });
  ...
  test('format', () => {
    const wrapper = mount(() => <Progress percentage={100} format={(percent: number) => `占比${percent}%`} />);
