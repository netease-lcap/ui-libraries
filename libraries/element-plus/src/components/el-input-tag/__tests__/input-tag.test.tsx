...
import { ComponentSize, EVENT_CODE } from 'element-plus/es/constants';
// import { InputTagInstance } from '@element-plus/element-plus';
import { ElFormItem } from 'element-plus/es/components/form';
import { ElInputTagPlus as InputTag } from '../index';

type InputTagInstance = any;
...
describe('InputTag.vue', () => {
  ...
  test('v-model', async () => {
    ...
    await wrapper.find('input').trigger('keydown', { code: EVENT_CODE.backspace });
    expect(wrapper.findAll('.el-tag').length).toBe(1);
    expect(wrapper.find('.el-tag').text()).toBe(AXIOM);
    expect(inputValue.value).toEqual([AXIOM]);
  ...
  test('trigger', async () => {
    const inputValue = ref<string[]>();
    const wrapper = mount(() => <InputTag v-model={inputValue.value} trigger="Space" />);
  ...
  test('size', async () => {
    ...
    const wrapper = mount(() => <InputTag modelValue={[AXIOM]} size={size.value} />);

    expect(wrapper.find('.el-input-tag').classes()).toContain('el-input-tag--default');
    expect(wrapper.find('.el-tag').classes()).toContain('el-tag--default');

    size.value = 'large';
    await nextTick();
    expect(wrapper.find('.el-input-tag').classes()).toContain('el-input-tag--large');
    expect(wrapper.find('.el-tag').classes()).toContain('el-tag--default');

    size.value = 'small';
    await nextTick();
    expect(wrapper.find('.el-input-tag').classes()).toContain('el-input-tag--small');
    expect(wrapper.find('.el-tag').classes()).toContain('el-tag--small');
  });

  test('disabled', async () => {
    const disabled = ref(true);
    const wrapper = mount(() => <InputTag modelValue={[AXIOM]} disabled={disabled.value} />);
  ...
  test('placeholder', async () => {
    const inputValue = ref<string[]>();
    const wrapper = mount(() => <InputTag v-model={inputValue.value} placeholder={AXIOM} />);

    expect(wrapper.find('input').element.placeholder).toMatchInlineSnapshot(`"${AXIOM}"`);
  ...
  test('readonly', async () => {
    ...
    const wrapper = mount(() => <InputTag modelValue={[AXIOM]} readonly={readonly.value} />);
  ...
  test('tagType', async () => {
    ...
    const wrapper = mount(() => <InputTag modelValue={[AXIOM]} tagType={type.value} />);
  ...
  test('tagEffect', async () => {
    ...
    const wrapper = mount(() => <InputTag modelValue={[AXIOM]} tagEffect={effect.value} />);
  ...
  describe('events', () => {
    ...
    test('update:modelValue', async () => {
      ...
      const inputValue = ref<string[]>();
      const wrapper = mount(() => <InputTag v-model={inputValue.value} onUpdate:modelValue={handleModelValue} />);
    ...
    test('change', async () => {
      ...
      const inputValue = ref<string[]>();
      const wrapper = mount(() => <InputTag v-model={inputValue.value} onChange={handleChange} />);
    ...
    test('add-tag', async () => {
      ...
      const inputValue = ref<string[]>();
      const wrapper = mount(() => <InputTag v-model={inputValue.value} onAdd-tag={handleTagAdd} />);
    ...
    test('remove-tag', async () => {
      ...
      const wrapper = mount(() => <InputTag v-model={inputValue.value} onRemove-tag={handleTagRemove} />);
  ...
  describe('slots', () => {
    test('prefix', async () => {
      const wrapper = mount(() => <InputTag v-slots={{ prefix: () => AXIOM }} />);

      expect(wrapper.find('.el-input-tag__prefix').text()).toBe(AXIOM);
    });

    test('suffix', async () => {
      const wrapper = mount(() => <InputTag v-slots={{ suffix: () => AXIOM }} />);
    ...
    test('tag', async () => {
      const wrapper = mount(() => (
        <InputTag modelValue={[AXIOM]} v-slots={{ tag: ({ value }: { value: string }) => `-${value}-` }} />
      ));
