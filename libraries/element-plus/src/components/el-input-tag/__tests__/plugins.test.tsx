...
import '@/utils/index';
import InputTagBasicAccumulate from '../plugins/index';
import InputTagIdeAccumulate from '../plugins/ide';
...
describe('el-input-tag plugins', () => {
  ...
  describe('index.tsx', () => {
    describe('InputTagBasicAccumulate 基础功能', () => {
      it('应该是一个有效的 PluginAccumulateTypes 实例', () => {
        expect(InputTagBasicAccumulate).toBeDefined();
        expect(typeof InputTagBasicAccumulate.addPlugin).toBe('function');
        expect(typeof InputTagBasicAccumulate.getPluginMethod).toBe('function');
        expect(Array.isArray(InputTagBasicAccumulate.Plugin)).toBe(true);
      });

      it('应该包含多个插件', () => {
        const plugins = InputTagBasicAccumulate.getPluginMethod();
        ...
        pluginNames.forEach((name) => {
          const plugin = InputTagBasicAccumulate.getPluginMethodByName(name);
    ...
    describe('handleDefaultPrps 插件功能测试', () => {
      const plugin = InputTagBasicAccumulate.getPluginMethodByName('handleDefaultPrps') as any;
      ...
      it('应该正确设置 formTagName 和 tagName', () => {
        ...
        expect(result.formTagName).toBe('el-form-input-tag');
        expect(result.tagName).toBe('el-input-tag');
      });
    ...
    describe('handlePreview 插件功能测试', () => {
      const plugin = InputTagBasicAccumulate.getPluginMethodByName('handlePreview') as any;
  ...
  describe('ide.ts', () => {
    describe('InputTagIdeAccumulate 基础功能', () => {
      it('应该是一个有效的 PluginAccumulateTypes 实例', () => {
        expect(InputTagIdeAccumulate).toBeDefined();
        expect(typeof InputTagIdeAccumulate.addPlugin).toBe('function');
        expect(typeof InputTagIdeAccumulate.getPluginMethod).toBe('function');
        expect(Array.isArray(InputTagIdeAccumulate.Plugin)).toBe(true);
      });

      it('应该包含 handleNodePath 插件', () => {
        const plugins = InputTagIdeAccumulate.getPluginMethod();
        expect(plugins).toHaveLength(0);

        const handleNodePathPlugin = InputTagIdeAccumulate.getPluginMethodByName('handleNodePath');
    ...
    describe('handleNodePath 插件功能测试', () => {
      const plugin = InputTagIdeAccumulate.getPluginMethodByName('handleNodePath') as any;
  ...
  describe('插件集成和扩展性测试', () => {
    it('应该能够与其他插件组合使用', () => {
      // 测试插件链式调用
      const combinedAccumulate = InputTagBasicAccumulate.addPlugin({
        name: 'testPlugin',
        handle: () => ({
    ...
    it('应该正确处理插件的执行顺序', () => {
      ...
      const testAccumulate = InputTagBasicAccumulate.addPlugin(plugin1).addPlugin(plugin2);
    ...
    it('应该正确处理插件方法的获取', () => {
      const allMethods = InputTagBasicAccumulate.getPluginMethod();
    ...
    it('应该正确处理不存在的插件查询', () => {
      const nonExistentPlugin = InputTagBasicAccumulate.getPluginMethodByName('nonExistent');
