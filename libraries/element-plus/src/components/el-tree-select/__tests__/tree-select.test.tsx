...
import { ElTreeSelectPlus as TreeSelect } from '../index';
...
const createComponent = ({
      ...
      render() {
        return (
          <TreeSelect
            ...
            ref={(val: InstanceType<typeof TreeSelect>) => (wrapperRef.value = val)}
            v-slots={slots}
          />
        );
      },
  ...
  return {
    wrapper,
    getWrapperRef: () => new Promise<InstanceType<typeof TreeSelect>>((resolve) => nextTick(() => resolve(wrapperRef.value!))),
    select: wrapper.findComponent({ name: 'ElSelect' }) as VueWrapper<InstanceType<typeof ElSelect>>,
    tree: wrapper.findComponent({ name: 'ElTree' }) as VueWrapper<InstanceType<typeof ElTree>>,
  };
};
...
describe('TreeSelect.vue', () => {
  ...
  test('modelValue', async () => {
    ...
    await tree.findAll('.el-select-dropdown__item').slice(-1)[0].trigger('click');
    await nextTick();
  ...
  test('disabled', async () => {
    ...
    await tree.find('.el-tree-node .el-select-dropdown__item.is-disabled').trigger('click');
    await nextTick();
  ...
  test('renderContent', async () => {
    const { tree } = createComponent({
      props: {
        renderContent: (h: RenderFunction, { data }: { data: { label: string } }) => {
  ...
  test('check-strictly showCheckbox click node', async () => {
    ...
    await tree.findAll('.el-tree-node__content .el-checkbox')[0].trigger('click');
    await nextTick();
    expect(select.vm.modelValue).toEqual([1]);
    expect(wrapperRef.getCheckedKeys()).toEqual([1]);
  });
  ...
  test('only show checkbox', async () => {
    ...
    await tree.find('.el-tree-node__content .el-checkbox__original').trigger('click');
    await nextTick();
  ...
  test('expand-on-click-node', async () => {
    ...
    await tree.findAll('.el-tree-node__content')[0].trigger('click');
    expect(tree.findAll('.el-tree-node__children')[0].attributes('style')).toContain('display: none;');

    await wrapper.setProps({ expandOnClickNode: true });
    await tree.findAll('.el-tree-node__content')[0].trigger('click');
    expect(tree.findAll('.el-tree-node__children')[0].attributes('style')).not.toContain('display: none;');
  });
  ...
  describe('filter method and remote method', () => {
      ...
      data.value = [
        {
          value: keywords,
          label: keywords,
          children: [{ value: `${keywords}-child`, label: `${keywords}-child` }],
        },
      ];
    };
    const testMethod = async (props: any) => {
      ...
      const testKeywords = async (keywords: string) => {
        ...
        expect(tree.findAll('.el-select-dropdown__item').map((item) => item.text())).toEqual([
          keywords,
          `${keywords}-child`,
        ]);
      ...
      // check first child
      await tree
        .findAll(props.showCheckbox ? '.el-checkbox__original' : '.el-select-dropdown__item')[1]
        .trigger('click');
  ...
  test('check/check-change events can accept correct params', async () => {
    ...
    const { select, tree } = createComponent({
      props: {
        showCheckbox: true,
        checkStrictly: true,
        onCheck: (node: any, { checkedKeys, checkedNodes, halfCheckedKeys, halfCheckedNodes }: any) => onCheck(
            node.value,
            checkedKeys,
  ...
  test('cached checked node and use lazy multiple', async () => {
    ...
    const node1Checkbox = tree.findAll('.el-tree-node__content')[0].find('.el-checkbox');
    await node1Checkbox.trigger('click');
    await nextTick();
    expect(select.vm.modelValue).toEqual([5, 1]);

    const node2Checkbox = tree.findAll('.el-tree-node__content')[1].find('.el-checkbox');
    await node2Checkbox.trigger('click');
    await nextTick();
  ...
  test('v-model source change', async () => {
    ...
    const wrapper = mount({
      ...
      template:
        '<TreeSelect v-for="item in data" v-model="item.value" :data="options" @update:modelValue="item.handleChange" />',
    });
  ...
  test('always focus when using filters', async () => {
    ...
    // checkbox click
    await tree.findAll('.el-tree-node__content .el-checkbox')[1].trigger('click');
