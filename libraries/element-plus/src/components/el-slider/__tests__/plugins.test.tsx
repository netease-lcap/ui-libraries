...
import SliderIdeAccumulate from '../plugins/ide';

describe('el-slider plugins', () => {
  beforeEach(() => {
    // 重置所有 mock
    vi.clearAllMocks();
  });
  ...
  describe('index.tsx', () => {
    describe('SliderAccumulate 基础功能', () => {
      it('应该是一个有效的 PluginAccumulateTypes 实例', () => {
        expect(SliderAccumulate).toBeDefined();
        expect(typeof SliderAccumulate.addPlugin).toBe('function');
        expect(typeof SliderAccumulate.getPluginMethod).toBe('function');
        expect(Array.isArray(SliderAccumulate.Plugin)).toBe(true);
      });
      ...
      it('应该包含所有必要的插件', () => {
        ...
        pluginNames.forEach((name) => {
          const plugin = SliderAccumulate.getPluginMethodByName(name);
    ...
    describe('handleTagName 插件功能测试', () => {
      const plugin = SliderAccumulate.getPluginMethodByName('handleTagName') as any;
    ...
    describe('handleComponentInForm 插件功能测试', () => {
      const plugin = SliderAccumulate.getPluginMethodByName('handleComponentInForm') as any;
    ...
    describe('handleControllableValue 插件功能测试', () => {
      const plugin = SliderAccumulate.getPluginMethodByName('handleControllableValue') as any;
    ...
    describe('handleHeight 插件功能测试', () => {
      const plugin = SliderAccumulate.getPluginMethodByName('handleHeight') as any;
      ...
      it('应该正确处理负数 height', () => {
        ...
        expect(result.height).toBe('-10px');
      });
      ...
      it('应该正确处理小数 height', () => {
        ...
        expect(result.height).toBe('10.5px');
      });
    });

    describe('handlePreview 插件功能测试', () => {
      const plugin = SliderAccumulate.getPluginMethodByName('handlePreview') as any;
    ...
    describe('插件集成和扩展性测试', () => {
      it('应该能够与其他插件组合使用', () => {
        const combinedAccumulate = SliderAccumulate.addPlugin({
          name: 'testPlugin',
          handle: () => ({
      ...
      it('应该正确处理插件的执行顺序', () => {
        ...
        const testAccumulate = SliderAccumulate.addPlugin(plugin1).addPlugin(plugin2);
      ...
      it('应该正确处理插件方法的获取', () => {
        const allMethods = SliderAccumulate.getPluginMethod();
      ...
      it('应该正确处理不存在的插件查询', () => {
        const nonExistentPlugin = SliderAccumulate.getPluginMethodByName('nonExistent');
  ...
  describe('ide.ts', () => {
    describe('SliderIdeAccumulate 基础功能', () => {
      it('应该是一个有效的 PluginAccumulateTypes 实例', () => {
        expect(SliderIdeAccumulate).toBeDefined();
        expect(typeof SliderIdeAccumulate.addPlugin).toBe('function');
        expect(typeof SliderIdeAccumulate.getPluginMethod).toBe('function');
        expect(Array.isArray(SliderIdeAccumulate.Plugin)).toBe(true);
      });

      it('应该包含 handleNodePath 插件', () => {
        const plugins = SliderIdeAccumulate.getPluginMethod();
        expect(plugins).toHaveLength(0);

        const handleNodePathPlugin = SliderIdeAccumulate.getPluginMethodByName('handleNodePath');
    ...
    describe('handleNodePath 插件功能测试', () => {
      const plugin = SliderIdeAccumulate.getPluginMethodByName('handleNodePath') as any;
      ...
      it('应该正确处理插件基本结构', () => {
        ...
        expect(result.class).toContain('Slider_');
      });
      ...
      it('应该正确处理空的 class', () => {
        ...
        expect(result.class).toContain('Slider_');
        expect(result.class.trim()).toMatch(/^Slider_\d+$/);
      });
    ...
    describe('插件集成和扩展性测试', () => {
      it('应该能够与其他插件组合使用', () => {
        const combinedAccumulate = SliderIdeAccumulate.addPlugin({
          name: 'testPlugin',
          handle: () => ({
            testProperty: 'test-value',
            customData: 'custom',
      ...
      it('应该正确处理插件的执行顺序', () => {
        ...
        const testAccumulate = SliderIdeAccumulate.addPlugin(plugin1).addPlugin(plugin2);
      ...
      it('应该正确处理插件方法的获取', () => {
        const allMethods = SliderIdeAccumulate.getPluginMethod();
        expect(Array.isArray(allMethods)).toBe(true);
        expect(allMethods.length).toBeGreaterThan(0);
      ...
      it('应该正确处理不存在的插件查询', () => {
        const nonExistentPlugin = SliderIdeAccumulate.getPluginMethodByName('nonExistent');
        expect(nonExistentPlugin).toBeUndefined();
      });
    });
  });
