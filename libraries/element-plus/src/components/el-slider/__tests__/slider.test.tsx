...
import { ElSlider as Slider } from '../index';
...
describe('Slider', () => {
  ...
  it('hide tooltip', () => {
    ...
    const wrapper = mount({
      setup() {
        return () => <Slider ref="slider" v-model={value.value} show-tooltip={false} />;
      },
    });
  ...
  describe('drag', () => {
    it('horizontal', async () => {
      ...
      vi.spyOn(wrapper.find('.el-slider__runway').element, 'clientWidth', 'get').mockImplementation(() => 200);
    ...
    it('vertical', async () => {
      ...
      const slider = wrapper.findComponent({ name: 'ElSliderButton' });
      vi.spyOn(wrapper.find('.el-slider__runway').element, 'clientHeight', 'get').mockImplementation(() => 200);
  ...
  describe('precision accuracy 3', () => {
    const value = ref(0);
    const wrapper = mount(() => <Slider showInput min={1} max={20} step={0.001} v-model={value.value} />);
    ...
    mount(
      {
        setup() {
          ...
          return () => show.value && <Slider v-model={value.value} min={10} show-input />;
        },
      },
      {
        attachTo: document.body,
  ...
  describe('range', () => {
    ...
    it('responsive to dynamic min and max', async () => {
      ...
      mount(() => <Slider v-model={value.value} min={min.value} max={max.value} range />);
    ...
    it('show stops', async () => {
      ...
      const wrapper = mount(() => <Slider v-model={value.value} step={10} range show-stops />);
    ...
    it('marks', async () => {
      ...
      const wrapper = mount(() => (
        <Slider v-model={value.value} min={20} step={10} marks={marksValue.value} range show-stops />
      ));
      ...
      expect(getComputedStyle(marks[marks.length - 1].element).color).toBe('rgb(255, 85, 0)');
    });
  });
