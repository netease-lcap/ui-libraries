...
import { ElDatePickerPlus as DatePicker } from '../index';
...
describe('Datetime Picker', () => {
  ...
  it('both picker show correct value', async () => {
    ...
    const wrapper = _mount(() => <DatePicker v-model={value.value} type="datetime" />);
    ...
    let spinners = document.querySelectorAll('.el-time-spinner ul li.is-active') as NodeListOf<HTMLElement>;
    ...
    await nextTick();
    spinners = document.querySelectorAll('.el-time-spinner ul li.is-active') as NodeListOf<HTMLElement>;
  ...
  it('time-picker select && input time && input date', async () => {
    ...
    input.trigger('focus');
    await nextTick();
    const input_ = document.querySelectorAll('.el-date-picker__editor-wrap input')[1] as HTMLElement;
    input_.focus();
    await nextTick();
    ...
    const button: HTMLElement = document.querySelector('.el-time-panel .confirm')!;
    button.click();
    await nextTick();
    ...
    const dateInput: HTMLInputElement = document.querySelector('.el-date-picker__editor-wrap input')!;
  ...
  it('now button: can not choose disabled date', async () => {
    ...
    const wrapper = _mount(() => <DatePicker v-model={value.value} type="datetime" disabledDate={disabledDate} />);
    ...
    await nextTick();
    // click now button
    const btn: HTMLElement = document.querySelector('.el-picker-panel__footer .is-text')!;
    btn.click();
    await nextTick();
  ...
  it('now button: should be disabled when current date is disabled', async () => {
    const isDisable = true;
    const disabledDate = () => isDisable;
    const wrapper = _mount(() => <DatePicker type="datetime" disabledDate={disabledDate} />);
  ...
  it('confirm button honors picked date', async () => {
    ...
    (document.querySelector('.el-date-picker__header .arrow-right') as HTMLElement).click();
    (document.querySelector('.el-date-picker__header .d-arrow-right') as HTMLElement).click();
  ...
  it('selectableRange', async () => {
    ...
    const disabledHours = Array.from(hoursEl.querySelectorAll('.is-disabled')).map((node) => Number(node.textContent));
    expect(disabledHours).toStrictEqual(disabledHoursArr);
    const minutesEl = list[1];
    const disabledMinutes = Array.from(minutesEl.querySelectorAll('.is-disabled')).map((node) => Number(node.textContent));
  ...
  it('defaultTime takes effect when the type is datetime', async () => {
    ...
    const wrapper = _mount(() => <DatePicker v-model={value.value} type="datetime" default-time={defaultTime.value} />);
    ...
    // time spinner highlight is correct
    const spinners = document.querySelectorAll('.el-time-spinner ul li.is-active') as NodeListOf<HTMLElement>;
  ...
  it('defaultTime only takes effect when time is not selected', async () => {
    ...
    timeInput.focus();
    await nextTick();
    const spinner: HTMLElement = document.querySelector('.el-time-spinner ul li.is-active')!;
...
describe('Datetimerange', () => {
  ...
  it('input date', async () => {
    ...
    const wrapper = _mount(() => <DatePicker v-model={value.value} type="datetimerange" />);
  ...
  it('clear button should empty the input value', async () => {
    ...
    dateCell[3].click();
    await nextTick();
    const headerValue = document.querySelectorAll<HTMLInputElement>('.el-date-range-picker__time-header input');
    expect(headerValue[0].value).not.toBe('');
    expect(headerValue[1].value).not.toBe('');
    const clearBtn = document.querySelectorAll<HTMLButtonElement>('.el-picker-panel__footer button')[0];
    clearBtn.click();
    await nextTick();
    input.trigger('blur');
    await nextTick();
  ...
  it('selectableRange', async () => {
    ...
    const wrapper = _mount(() => (
      <DatePicker v-model={value.value} type="datetimerange" disabledHours={disabledHoursData} />
    ));
    ...
    const disabledHours2 = Array.from(hoursEl2.querySelectorAll('.is-disabled')).map((node) => Number(node.textContent));
  ...
  it('select same date, different time', async () => {
    ...
    await nextTick();
    const rightList = document.querySelectorAll('.is-right .el-time-spinner__list') as any;
    // auto set left time to right time
    expect(rightList[0]!.querySelector('.el-time-spinner__item.is-active').innerHTML).toBe(leftSelect[0]);
    expect(rightList[1].querySelector('.el-time-spinner__item.is-active').innerHTML).toBe(leftSelect[1]);
    expect(rightList[2].querySelector('.el-time-spinner__item.is-active').innerHTML).toBe(leftSelect[2]);
    ...
    expect(value.value.map((_) => dayjs(_).format('HH:mm:ss'))).toStrictEqual(['10:59:59', '12:12:12']);
  });
  ...
  describe('form item accessibility integration', () => {
    it('automatic id attachment', async () => {
      ...
      expect(formItemLabel.attributes().for).toBe(datePickerInput.attributes().id);
    });
  ...
  it('prop defaultTime should not confilt with prop shortcuts', async () => {
    ...
    input.trigger('focus');
    await nextTick();
    (document.querySelector('.el-picker-panel__sidebar .el-picker-panel__shortcut') as HTMLElement).click();
    await nextTick();
    expect(value.value).toBeDefined();
    expect(dayjs(value.value).format('YYYY-MM-DD HH:mm:ss')).toStrictEqual('2023-01-01 12:00:00');
  });
});
