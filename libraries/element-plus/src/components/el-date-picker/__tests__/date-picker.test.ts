...
import { CommonPicker } from 'element-plus/es/components/time-picker';
...
describe('DatePicker', () => {
  ...
  it('select date', async () => {
    ...
    const arrowLeftElm = document.querySelector('.el-date-picker__prev-btn .arrow-left') as HTMLElement;
    const arrowRightElm = document.querySelector('.el-date-picker__next-btn .arrow-right') as HTMLElement;
    expect(spans[0].textContent).toContain(date.year());
    expect(spans[1].textContent).toContain(date.format('MMMM'));
    const arrowLeftYeayElm = document.querySelector('.el-date-picker__prev-btn .d-arrow-left') as HTMLElement;
  ...
  it('select year picker when using disabledDate prop', async () => {
    ...
    input.trigger('focus');
    await nextTick();
    const yearLabel: HTMLElement = document.querySelectorAll('.el-date-picker__header-label')[0];
    yearLabel.click();
    await nextTick();
  ...
  it('select month picker when using disabledDate prop', async () => {
    ...
    input.trigger('focus');
    await nextTick();
    const monthLabel: HTMLElement = document.querySelectorAll('.el-date-picker__header-label')[1];
    monthLabel.click();
    await nextTick();
  ...
  it('custom content', async () => {
    ...
    await nextTick();
    {
      (document.querySelector('td.available .el-date-table-cell__text') as HTMLElement).click();
    }
    input.trigger('focus');
    await nextTick();
    expect(document.querySelector('td.available .el-date-table-cell__text').classList.contains('current')).toBeTruthy();
  });
  ...
  describe('value-format', () => {
    it('with literal string', async () => {
      ...
      expect(vm.value).toBe(
        dayjs(`[Element-Plus] 01/${`0${day.month() + 1}`.slice(-2)} ${day.year()}`, valueFormat).format(valueFormat),
      );
...
describe('MonthPicker', () => {
  ...
  />`,
    ...
    (document.querySelector('.el-month-table .el-date-table-cell__text') as HTMLElement).click();
    await nextTick();
    const vm = wrapper.vm as any;
...
describe('YearPicker', () => {
  ...
  />`,
    ...
    await nextTick();
    (document.querySelector('.el-year-table .el-date-table-cell__text') as HTMLElement).click();
    await nextTick();
    const vm = wrapper.vm as any;
  ...
  it('value-format', async () => {
    ...
    input.trigger('focus');
    await nextTick();
    const cell = document.querySelector('.el-year-table .el-date-table-cell__text') as HTMLElement;
    cell.click();
    await nextTick();
...
describe('WeekPicker', () => {
  ...
  />`,
    ...
    await nextTick();
    (document.querySelectorAll('.el-month-table .el-date-table-cell__text')[7] as HTMLElement).click();
    await nextTick();
    expect(document.querySelector('.is-week-mode')).not.toBeNull();
...
describe('DatePicker dates', () => {
  ...
  it('selected', async () => {
    ...
    input.trigger('focus');
    await nextTick();
    expect(document.querySelectorAll('.el-date-table__row .selected').length).toBe(1);
  });
});
...
describe('DatePicker months', () => {
  ...
  />`,
    ...
    input.trigger('focus');
    await nextTick();
    const td = document.querySelectorAll('.el-month-table tr td') as NodeListOf<HTMLElement>;
    const vm = wrapper.vm as any;
    td[0].click();
    await nextTick();
    expect(vm.value.length).toBe(1);
    ...
    await nextTick();
    expect(vm.value.length).toBe(2);
    expect(document.querySelectorAll('.el-month-table tr .current').length).toBe(2);
    td[0].click();
    await nextTick();
    expect(vm.value.length).toBe(1);
    td[1].click();
  ...
  it('selected', async () => {
    ...
    input.trigger('focus');
    await nextTick();
    expect(document.querySelectorAll('.el-month-table tr .current').length).toBe(1);
  });
});
...
describe('DateRangePicker', () => {
  ...
  it('value-format', async () => {
    ...
    />`,
      () => ({
        value: [dayjs(new Date(2021, 4, 2)).format(valueFormat), dayjs(new Date(2021, 4, 12)).format(valueFormat)],
      }),
    );
    await nextTick();
...
describe('MonthRange', () => {
  ...
  it('prop defaultTime should not confilt with prop shortcuts', async () => {
    ...
    input.trigger('focus');
    await nextTick();
    document.querySelector('.el-picker-panel__sidebar .el-picker-panel__shortcut').click();
    await nextTick();
    const vm = wrapper.vm as any;
    expect(vm.value).toBeDefined();
