...
import { ElDescriptionsPlus as ElDescriptions, ElDescriptionsItemPlus as ElDescriptionsItem } from '../index';
...
describe('Descriptions.vue', () => {
  ...
  test('should render class props', () => {
    const wrapper = mount(() => (
      <ElDescriptions border>
        {Array.from({ length: 3 }).map(() => (
          <ElDescriptionsItem class-name="class-name" label-class-name="label-class-name" />
        ))}
      </ElDescriptions>
    ));
    ...
    const wrapper = mount(() => (
      <ElDescriptions column={5} direction={direction.value} border>
        {Array.from({ length: 10 }).map((item) => (
          <ElDescriptionsItem label={String(item)}>{String(item)}</ElDescriptionsItem>
        ))}
      </ElDescriptions>
    ));
  ...
  test('should render title slots', async () => {
    const wrapper = mount(() => (
      <ElDescriptions
        v-slots={{
          title: () => 'title',
          default: () => Array.from({ length: 10 }).map(() => <ElDescriptionsItem />),
        }}
      />
    ));
  ...
  test('should render labelWidth prop of DescriptionsItem', () => {
    ...
    expect(wrapper.find('.el-descriptions__label').attributes('style')).toContain('width: 150px');
  });
  ...
  test('should render labelWidth prop of Descriptions and DescriptionsItem with higher priority', () => {
    ...
    expect(wrapper.findAll('.el-descriptions__label')[0].attributes('style')).toContain('width: 100px');
    expect(wrapper.findAll('.el-descriptions__label')[1].attributes('style')).toContain('width: 150px');
  });
