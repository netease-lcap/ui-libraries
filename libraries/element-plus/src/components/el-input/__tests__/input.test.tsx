...
describe('Input.vue', () => {
  ...
  test('sets value on textarea / input type change', async () => {
    ...
    expect(vm.$el.querySelector('textarea').value).toMatchInlineSnapshot('"123"');
    type.value = 'password';
    await nextTick();
    await nextTick();
  ...
  test('use formatter and parser', () => {
    ...
    const wrapper = mount(() => <Input v-model={val.value} formatter={formatter} parser={parser} />);
  ...
  describe('Input Events', () => {
    ...
    test('textarea & event:focus', async () => {
      const content = ref('');
      const wrapper = mount(() => (
        <Input type="textarea" placeholder="请输入内容" modelValue={content.value} onFocus={handleFocus} />
      ));
    ...
    test('textarea & event:blur', async () => {
      const content = ref('');
      const wrapper = mount(() => (
        <Input type="textarea" placeholder="请输入内容" modelValue={content.value} onBlur={handleBlur} />
      ));
