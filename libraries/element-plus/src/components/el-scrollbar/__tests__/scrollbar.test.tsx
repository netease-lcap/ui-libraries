...
import makeScroll from '@ep-test/test-utils/make-scroll';
import defineGetter from '@ep-test/test-utils/define-getter';
import { ElScrollbar as Scrollbar } from '../index';
...
describe('ScrollBar', () => {
  test('vertical', async () => {
    ...
    expect(wrapper.find('.is-vertical div').attributes('style')).toContain('transform: translateY(50%); height: 80px;');
  ...
  test('set scrollTop & scrollLeft', async () => {
    ...
    const wrapper = mount({
      setup() {
        return () => (
          <Scrollbar ref="scrollbar" style={`height: ${outerHeight}px; width: ${outerWidth}px;`} always>
            <div style={`height: ${innerHeight}px; width: ${innerWidth}px;`}></div>
          </Scrollbar>
        );
      },
    ...
    await nextTick();
    expect(wrapper.find('.is-vertical div').attributes('style')).toContain('transform: translateY(0%); height: 80px;');
    expect(wrapper.find('.is-horizontal div').attributes('style')).toContain('transform: translateX(0%); width: 80px;');
  ...
  test('should render min-size props', async () => {
    ...
    expect(wrapper.find('.is-vertical div').attributes('style')).toContain('transform: translateY(0%); height: 20px;');
    offsetHeightRestore();
    scrollHeightRestore();
  });
