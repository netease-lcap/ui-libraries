...
export default FormItemPluginAccumulate.addPlugin({
  ...
  .addPlugin({
    name: 'handleColSpan',
    handle(props) {
      const colSpanProp = props.get('colSpan') ?? 1;
      const inject = props.get('inject');
      const formColumns = Number(inject?.[$formProvide]?.columns);
      const requested = Math.max(1, Math.floor(Number(colSpanProp)) || 1);
      // 超过表单总列数时撑满整行
      const colSpan = Number.isFinite(formColumns) && formColumns > 0 ? Math.min(requested, formColumns) : requested;
      const style = props.get('style') ?? {};
      const deletePropsList = ((props.get($deletePropsList) as unknown as string[]) ?? []).concat('colSpan');

      return {
        style: {
          ...(_.isPlainObject(style) ? style : {}),
          '--el-form-item-col-span': colSpan,
        },
