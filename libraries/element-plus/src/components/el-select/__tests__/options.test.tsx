...
import { ElSelect as Select, ElOption as Options } from '../index';
...
describe('options', () => {
  ...
  it('renders emit correct options', async () => {
    createWrapper({
      default: () => samples.map((_, i) => <ElOptionStub label={getLabel(i)} />),
    });
  ...
  it('renders emit correct options with option group', async () => {
    createWrapper({
      default: () => samples.map((_, i) => (
        <ElOptionGroupStub label={getLabel(i)}>
          {{
              default: () => samples.map((_, j) => <ElOptionStub label={getLabel(`${i}-${j}`)} value={j} />),
            }}
        </ElOptionGroupStub>
        )),
    });
