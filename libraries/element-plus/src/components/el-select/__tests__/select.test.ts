...
describe('Select', () => {
  ...
  //   expect(wrapper.find(`.${WRAPPER_CLASS_NAME}`).classes()).toContain('is-focused');

  //   await wrapper.find('button').trigger('mousedown');
  //   await wrapper.find('button').trigger('mouseup');

  //   expect(wrapper.find(`.${WRAPPER_CLASS_NAME}`).classes()).not.toContain('is-focused');
  //   expect(handleBlur).toHaveBeenCalledTimes(1);
  // });
  ...
  describe('should show all options when open select dropdown', () => {
    ...
    //   await wrapper.find(`.${WRAPPER_CLASS_NAME}`).trigger('click');
    //   const { vm } = wrapper.findComponent(Select);
