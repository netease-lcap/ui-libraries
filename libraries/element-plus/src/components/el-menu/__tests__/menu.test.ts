...
import {
  ElMenuPlus as Menu,
  ElMenuItemGroup as MenuGroup,
  ElMenuItem as MenuItem,
  ElSubMenu as SubMenu,
} from '../index';

const _mount = (template: string, options = {}) => mount({
    components: {
      'el-menu': Menu,
...
describe('other', () => {
  ...
  test('menu group', async () => {
    ...
    expect(group1.vm.$el.querySelector('.el-menu-item-group__title').innerHTML).toEqual('分组一');
  });
  test('dynamic menus, issue 9092', async () => {
    ...
    await nextTick();
    expect(instance.$el.querySelector('.el-menu-item.is-active').innerHTML).toEqual('new');
  });
});
