...
namespace nasl.ui {
  ...
  export class TreeSelectOptions<T, V, M extends nasl.core.Boolean> extends ViewComponentOptions {
    ...
    //   description: '清空前触发',
    // })
    // onBeforeClear: (event: { value: V; oldValue: V; values: nasl.collection.List<V>; oldValues: nasl.collection.List<V> }) => any;

    // @Event({
    //   title: '清空后',
    //   description: '清空时触发',
    // })
    // onClear: (event: { value: V; oldValue: V; values: nasl.collection.List<V>; oldValues: nasl.collection.List<V> }) => any;
