...
namespace nasl.ui {
  ...
  export class Descriptions extends ViewComponent {
    constructor(options?: Partial<DescriptionsOptions>) {
      super();
    }
  }

  export class DescriptionsOptions extends ViewComponentOptions {
    @Prop({
      group: '主要属性',
      title: '列数',
    ...
    @Slot({
      title: '默认',
      description: '插入`<DescriptionsItem>`子组件。',
      snippets: [
        {
          title: '详情列表项',
          code: '<DescriptionsItem><template #label><Text text="基本信息"></Text></template><Text children="张三"></Text></DescriptionsItem>',
        },
      ],
    })
  ...
  export class DescriptionsItem extends ViewComponent {
