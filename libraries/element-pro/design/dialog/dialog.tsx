...
import {
  ...
  InfoCircleFilledIcon as ElInfoCircleFilledIcon,
...
import { DialogCloseContext, ElDialogProps } from './type';
  ...
  methods: {
    ...
    keyboardEvent(e: KeyboardEvent) {
      if (e.code === 'Escape' && stack.top === this.uid) {
        emitEvent<Parameters<ElDialogProps['onEscKeydown']>>(this, 'esc-keydown', { e });
    ...
    keyboardEnterEvent(e: KeyboardEvent) {
      ...
      if ((code === 'Enter' || code === 'NumpadEnter') && stack.top === this.uid) {
        emitEvent<Parameters<ElDialogProps['onConfirm']>>(this, 'confirm', { e });
      }
    },
    overlayAction(e: MouseEvent) {
      ...
      emitEvent<Parameters<ElDialogProps['onOverlayClick']>>(this, 'overlay-click', { e });
    ...
    closeBtnAction(e: MouseEvent) {
      emitEvent<Parameters<ElDialogProps['onCloseBtnClick']>>(this, 'close-btn-click', { e });
      this.emitCloseEvent({
        trigger: 'close-btn',
        e,
      });
    ...
    cancelBtnAction(e: MouseEvent) {
      emitEvent<Parameters<ElDialogProps['onCancel']>>(this, 'cancel', { e });
      this.emitCloseEvent({
        trigger: 'cancel',
        e,
      });
    ...
    // 打开弹窗动画结束时事件
    afterEnter() {
      emitEvent<Parameters<ElDialogProps['onOpened']>>(this, 'opened');
    },
    // 关闭弹窗动画结束时事件
    afterLeave() {
      if (this.isModeLess && this.draggable) {
      ...
      emitEvent<Parameters<ElDialogProps['onClosed']>>(this, 'closed');
      this.animationEnd = true;
    },

    emitCloseEvent(context: DialogCloseContext) {
      emitEvent<Parameters<ElDialogProps['onClose']>>(this, 'close', context);
      // 默认关闭弹窗
    ...
    getIcon() {
      const { InfoCircleFilledIcon, CheckCircleFilledIcon, ErrorCircleFilledIcon } = this.useGlobalIcon({
        InfoCircleFilledIcon: ElInfoCircleFilledIcon,
    ...
    renderDialog() {
      ...
      const footerContent = renderElNodeJSX(this, 'footer', defaultFooter);
            ...
            <div key="dialog" ref="dialog" class={this.dialogClass} style={this.dialogStyle}>
              <div class={headerClassName} onmousedown={this.onStopDown}>
                <div class={`${this.componentName}__header-content`}>
                  {this.getIcon()}
                  {renderElNodeJSX(this, 'header', defaultHeader)}
                </div>
                {this.closeBtn ? (
                  <span class={closeClassName} onClick={this.closeBtnAction}>
                    {renderElNodeJSX(this, 'closeBtn', defaultCloseBtn)}
                  </span>
                ) : null}
              </div>
