...
import ElButton, { ButtonProps } from '../button';
import { PopconfirmConfig, DialogConfig, DrawerConfig } from '../config-provider';
import { ClassName, ElNode } from '../common';

export type MixinsFooterButton = string | ButtonProps | ElNode;
...
export default Vue.extend({
  methods: {
    getConfirmBtn(options: MixinsConfirmBtn) {
      ...
      // 属性和插槽都不存在，就返回全局默认配置
      if (!confirmBtn && !this.$scopedSlots.confirmBtn) {
        return <ElButton class={className} props={{ loading: confirmLoading, ...defaultButtonProps }} />;
      }
      // 如果属性存在，优先返回属性配置
      ...
      return renderElNodeJSX(this, 'confirmBtn');
    },
    ...
    getCancelBtn(options: MixinsCancelBtn) {
      ...
      // 属性和插槽都不存在，就返回全局默认配置
      if (!cancelBtn && !this.$scopedSlots.cancelBtn) {
        return <ElButton class={className} props={{ ...defaultButtonProps }} />;
      }
      // 如果属性存在，优先返回属性配置
      ...
      // 渲染插槽 或 function 类型的 confirmBtn，属性优先级更高
      return renderElNodeJSX(this, 'cancelBtn');
    },
    ...
    getButtonByProps(
      ...
      return <ElButton class={className} props={newOptions} />;
    },
