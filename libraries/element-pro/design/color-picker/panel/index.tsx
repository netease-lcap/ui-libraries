...
import { ElColorPickerProps, ColorPickerChangeTrigger } from '../type';
import { ElColorModes } from '../interfaces';
...
export default defineComponent({
  ...
  setup(props) {
    ...
    const mode = ref<ElColorModes>(props.colorModes?.length === 1 ? props.colorModes[0] : 'monochrome');
    ...
    const formatModel = ref<ElColorPickerProps['format']>(color.value.isGradient ? 'CSS' : 'RGB');
    ...
    const addRecentlyUsedColor = () => {
      ...
      if (colors.length > EL_COLOR_USED_COLORS_MAX_SIZE) {
        colors.length = EL_COLOR_USED_COLORS_MAX_SIZE;
      }
     ...
     * @returns
     */
    const handleModeChange = (value: ElColorModes) => {
      mode.value = value;
     ...
     * @returns
     */
    const handleFormatModeChange = (format: ElColorPickerProps['format']) => (formatModel.value = format);
