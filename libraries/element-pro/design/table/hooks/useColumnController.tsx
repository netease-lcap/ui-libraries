...
import { SettingIcon as ElSettingIcon } from '@element-ui-icons';
...
import { PrimaryTableCol, ElPrimaryTableProps } from '../type';
...
export default function useColumnController(props: ElPrimaryTableProps, context: SetupContext) {
  const { classPrefix, globalConfig } = useConfig('table', props.locale);
  const { SettingIcon } = useGlobalIcon({ SettingIcon: ElSettingIcon });
  const {
          ...
          <div class={classes}>
            <div class={`${prefix}-table__column-controller-body`}>
              {isFunction(columnControllerTopContent)
                ...
                : renderElNodeJSX('columnControllerTopContent')}
              {/* 请选择需要在表格中显示的数据列 */}
              ...
              {isFunction(columnControllerBottomContent)
                ...
                : renderElNodeJSX('columnControllerBottomContent')}
            </div>
          </div>
        );
        return defaultNode;
