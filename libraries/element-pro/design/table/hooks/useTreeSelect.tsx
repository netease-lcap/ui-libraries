...
import {
  ElEnhancedTableProps, ElPrimaryTableProps, TableRowData, PrimaryTableCol,
} from '../type';
...
type SelectChangeParams = Parameters<ElPrimaryTableProps['onSelectChange']>;

export default function useTreeSelect(props: ElEnhancedTableProps, treeDataMap: Ref<TableTreeDataMap>) {
  const {
  ...
  const [tSelectedRowKeys, setElSelectedRowKeys] = useDefaultValue(
    selectedRowKeys,
  ...
  function onInnerSelectChange(rowKeys: SelectChangeParams[0], extraData: SelectChangeParams[1]) {
    if (!tree.value || tree.value.checkStrictly) {
      setElSelectedRowKeys(rowKeys, extraData);
      return;
    }
  ...
  function handleSelectAll(extraData: SelectChangeParams[1]) {
    ...
    setElSelectedRowKeys(newRowKeys, newExtraData);
  }
