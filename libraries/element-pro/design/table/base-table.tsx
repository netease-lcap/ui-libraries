...
import ElBody, { extendTableProps } from './tbody';
import { BaseTableProps } from './interface';
import { renderElNodeJSX, useElementLazyRender } from '../hooks';
import useStyle, { formatCSSUnit } from './hooks/useStyle';
import useClassName from './hooks/useClassName';
import { useConfig } from '../config-provider/useConfig';
import { Affix } from '../affix';
import { ROW_LISTENERS } from './tr';
import ElHead from './thead';
import ElFoot from './tfoot';
...
export default defineComponent({
  ...
  setup(props: BaseTableProps, context: SetupContext) {
    ...
    const scrollColumnIntoView = (colKey: string) => {
      ...
      const fixedThDom = tableContentRef.value.querySelectorAll('th.el-p-table__cell--fixed-left');
      let totalWidth = 0;
      ...
      const affixedHeader = Boolean((this.headerAffixedTop || isVirtual) && this.tableWidth) && (
        ...
        >
          <table class={this.tableElmClasses} style={{ ...this.tableElementStyles, width: `${this.tableElmWidth}px` }}>
            {colgroup}
            <ElHead scopedSlots={this.$scopedSlots} props={this.getHeadProps(true)} />
          </table>
        </div>
      );
  ...
  render(h) {
    ...
    const tableContent = (
      ...
      >
        ...
        >
          ...
          {this.showHeader && <ElHead scopedSlots={this.$scopedSlots} props={this.getHeadProps(false)} />}
          <ElBody ref="tableBodyRef" scopedSlots={this.$scopedSlots} props={tableBodyProps} on={tBodyListener} />
          <ElFoot
            rowKey={this.rowKey}
            scopedSlots={this.$scopedSlots}
            isFixedHeader={this.isFixedHeader}
    ...
    const topContent = renderElNodeJSX(this, 'topContent');
    const bottomContent = renderElNodeJSX(this, 'bottomContent');
