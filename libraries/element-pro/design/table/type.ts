...
export interface ElBaseTableProps<T extends TableRowData = TableRowData> {
  /**
   ...
   * 表格底部内容，可以用于自定义列设置等
   */
  bottomContent?: string | ElNode;
  /**
   * 单元格数据为空时呈现的内容
   */
  cellEmptyContent?: string | ElNode<BaseTableCellParams<T>>;
  /**
   * 列配置，泛型 T 指表格数据类型
   * @default []
   */
...
export interface BaseTableCol<T extends TableRowData = TableRowData> {
   ...
   */
  cell?: string | ElNode<BaseTableCellParams<T>>;
  /**
   * 用于多级表头，泛型 T 指表格数据类型
   */
  ...
  ellipsis?:
    | boolean
    | ElNode<BaseTableCellParams<T>>
    | TooltipProps
    | { props: TooltipProps; content: ElNode<BaseTableCellParams<T>> };
  /**
  ...
  ellipsisTitle?:
    | boolean
    | ElNode<BaseTableColParams<T>>
    | TooltipProps
    | { props: TooltipProps; content: ElNode<BaseTableColParams<T>> };
  /**
   * 固定列显示位置
   * @default left
   */
   ...
   */
  foot?: string | ElNode<{ col: BaseTableCol; colIndex: number }>;
  /**
   ...
   */
  render?: ElNode<BaseTableRenderParams<T>>;
  /**
   ...
   */
  title?: string | ElNode<{ col: BaseTableCol; colIndex: number }>;
  /**
...
export interface ElPrimaryTableProps<T extends TableRowData = TableRowData>
  extends Omit<ElBaseTableProps<T>, 'columns' | 'onCellClick'> {
  /**
   * 异步加载状态。值为 `loading` 显示默认文字 “正在加载中，请稍后”，值为 `loading-more` 显示“点击加载更多”，值为其他，表示完全自定义异步加载区域内容
   */
  asyncLoading?: 'loading' | 'load-more' | ElNode;
  /**
   ...
   * @default true
   */
  expandIcon?: boolean | ElNode<ExpandArrowRenderParams<T>>;
  /**
   * 是否允许点击行展开
   */
   ...
   * 展开行内容，泛型 T 指表格数据类型
   */
  expandedRow?: ElNode<TableExpandedRowParams<T>>;
  /**
   * 展开行
   * @default []
   */
   ...
   */
  filterIcon?: ElNode<{ col: PrimaryTableCol<T>; colIndex: number }>;
  /**
   * 自定义过滤状态行及清空筛选等
   */
  filterRow?: string | ElNode;
  /**
   * 过滤数据的值
   */
...
export interface PrimaryTableCol<T extends TableRowData = TableRowData>
   ...
   */
  cell?: string | ElNode<PrimaryTableCellParams<T>>;
  /**
   ...
   * 自定义表头或单元格，泛型 T 指表格数据类型
   */
  render?: ElNode<PrimaryTableRenderParams<T>>;
  /**
   ...
   * 自定义表头渲染。值类型为 Function 表示以函数形式渲染表头。值类型为 string 表示使用插槽渲染，插槽名称为 title 的值。优先级高于 render
   */
  title?: string | ElNode<{ col: PrimaryTableCol; colIndex: number }>;
  /**
...
export interface ElEnhancedTableProps<T extends TableRowData = TableRowData> extends ElPrimaryTableProps<T> {
  /**
   ...
   */
  treeExpandAndFoldIcon?: ElNode<{ type: 'expand' | 'fold'; row: T }>;
  /**
...
export interface TableColumnController {
   ...
   * 列配置控制器底部内容
   */
  columnControllerBottomContent?: ElNode;
  /**
   * 列配置控制器顶部内容
   */
  columnControllerTopContent?: ElNode;
  /**
   * 透传弹框组件全部特性，如：防止滚动穿透
   */
