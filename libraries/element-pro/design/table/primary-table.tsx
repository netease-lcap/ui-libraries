...
import { ElPrimaryTableProps, PrimaryTableCol, TableRowData } from './type';
...
export default defineComponent({
  ...
  setup(props: ElPrimaryTableProps, context) {
    ...
    const onInnerPageChange = (pageInfo: PageInfo, newData: Array<TableRowData>) => {
      ...
      const changeParams: Parameters<ElPrimaryTableProps['onChange']> = [
        { pagination: pageInfo },
    ...
    const onSingleRowClick: ElPrimaryTableProps['onRowClick'] = (params) => {
    ...
    const DURATION = 250;
    const onInnerRowClick: ElPrimaryTableProps['onRowClick'] = (params) => {
  ...
  methods: {
    ...
    formatNode(api: string, renderInnerNode: Function, condition: boolean, extra?: { reverse?: boolean }) {
      ...
      const propsNode = this.renderElNode(api);
