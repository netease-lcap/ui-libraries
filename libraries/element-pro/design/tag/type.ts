...
import { ElNode, SizeEnum } from '../common';
...
export interface ElCheckTagProps {
   ...
   * 透传标签选中态属性
   */
  checkedProps?: ElTagProps;
  /**
   * 组件子元素；传入数组时：[选中内容，非选中内容]
   */
  content?: string | number | string[] | ElNode;
  /**
   * 组件子元素，默认插槽
   */
  default?: string | ElNode;
...
export interface ElCheckTagGroupProps {
  /**
   * 透传标签选中态属性
   */
  checkedProps?: ElTagProps;
...
export interface CheckTagGroupOption extends ElCheckTagProps {
  label: string | ElNode;
  value: string | number;
}
