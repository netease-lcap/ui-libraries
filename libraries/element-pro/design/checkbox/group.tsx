...
import { CheckboxOptionObj, ElCheckboxProps, CheckboxGroupValue } from './type';
...
export default defineComponent({
  ...
  setup(props) {
    ...
    const getAllCheckboxValue = (): CheckboxGroupValue => {
      const val = new Set<ElCheckboxProps['value']>();
    ...
    const onCheckAllChange = (checked: boolean, context: { e: Event; source?: 'el-checkbox-pro' }) => {
    ...
    const handleCheckboxChange = (data: { checked: boolean; e: Event; option: ElCheckboxProps }) => {
      const currentValue = data.option.value;
      if (!isArray(innerValue.value)) {
        console.warn(`ElementUI CheckboxGroup Warn: \`value\` must be an array, instead of ${typeof innerValue.value}`);
        return;
      }
    ...
    const onCheckedChange = (p: { checked: boolean; checkAll: boolean; e: Event; option: ElCheckboxProps }) => {
