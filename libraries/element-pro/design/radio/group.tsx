...
import {
  ElRadioGroupProps, RadioOptionObj, RadioOption, RadioValue,
} from './type';
import Radio from './radio';
import RadioButton from './radio-button';
import { ElNodeReturnValue } from '../common';
...
export default mixins(classPrefixMixins).extend({
  ...
  render(h: CreateElement): VNode {
    ...
    let children: ElNodeReturnValue = $scopedSlots.default && $scopedSlots.default(null);
    ...
    if (this.options && this.options.length) {
      children = this.options.map((option: RadioOption, index) => {
        ...
        const RadioComponent = option.button ? RadioButton : Radio;
  ...
  methods: {
    ...
    handleRadioChange(value: RadioValue, context: { e: Event }): void {
      emitEvent<Parameters<ElRadioGroupProps['onChange']>>(this, 'change', value, context);
    },
