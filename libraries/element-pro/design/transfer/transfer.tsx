...
import { PageInfo, ElPaginationProps } from '../pagination/type';
...
export default mixins(getConfigReceiverMixins('transfer')).extend({
  ...
  computed: {
    ...
    titleOption(): TransferListOptionBase<string> {
      // TODO：此处需分别处理 Array 和 ElNode 内容
      // @ts-ignore
      return getTransferListOption<string>(this.title);
    },
    paginationOption(): TransferListOptionBase<ElPaginationProps> {
      return getTransferListOption<ElPaginationProps>(this.pagination);
    },
  },
  methods: {
    transferTo(toDirection: TransferListType) {
      ...
      emitEvent<Parameters<ElTransferProps['onChange']>>(this, 'change', newTargetValue, params);
    },
    // 点击移到右边按钮触发的函数
    transferToRight() {
    ...
    handleCheckedChange(val: Array<TransferValue>, listType: TransferListType) {
      ...
      emitEvent<Parameters<ElTransferProps['onCheckedChange']>>(this, 'checked-change', event);
    },
    filterMethod(
    ...
    handleScroll(e: Event, listType: TransferListType) {
      ...
      emitEvent<Parameters<ElTransferProps['onScroll']>>(this, 'scroll', event);
    },
    handleSearch(e: SearchEvent) {
      emitEvent<Parameters<ElTransferProps['onSearch']>>(this, 'search', e);
    },
    handlePageChange(pageInfo: PageInfo, listType: TransferListType) {
      emitEvent<Parameters<ElTransferProps['onPageChange']>>(this, 'page-change', pageInfo, { type: listType });
    },
