...
import { PageInfo, ElPaginationProps, Pagination as ElPagination } from '../../pagination';
import { Checkbox as ElCheckbox, CheckboxGroup as ElCheckboxGroup, CheckboxProps } from '../../checkbox';
import {
...
export default mixins(keepAnimationMixins, classPrefixMixins).extend({
  ...
  props: {
    ...
    transferItem: Function as PropType<ElTransferProps['transferItem']>,
    empty: {
  ...
  computed: {
    ...
    paginationProps(): ElPaginationProps {
      const defaultPaginationProps: ElPaginationProps = {
        size: 'small',
        theme: 'simple',
        totalContent: false,
        pageSizeOptions: [],
  ...
  methods: {
    ...
    renderTitle() {
      ...
      const titleNode = renderElNodeJSXDefault(this, 'title', {
        defaultNode,
        params: {
          type: this.listType,
        },
    ...
    renderContent() {
      ...
      if (!isDraggable) {
        defaultNode = (
          <ElCheckboxGroup value={this.checkedValue} onChange={this.handleCheckedChange}>
            {this.curPageData.map((item, index) => (
              ...
              >
                {renderElNodeJSXDefault(this, 'transferItem', {
      ...
      return (
        <div class={`${this.componentName}__list-content narrow-scrollbar`} onScroll={this.scroll}>
          {renderElNodeJSXDefault(rootNode, 'tree', {
            defaultNode,
            params: {
    ...
    renderEmpty() {
      ...
      return (
        <div class={`${this.componentName}__empty`}>
          {renderElNodeJSXDefault(this, 'empty', {
            defaultNode,
            params: {
              type: this.listType,
            },
    ...
    renderFooter() {
      ...
      return renderElNodeJSXDefault(this, 'footer', {
        defaultNode,
        params: {
          type: this.listType,
        },
      ...
      <div class={`${this.componentName}__list ${this.componentName}__list-${this.listType}`}>
        ...
        {this.pagination && this.pageSize > 0 && this.pageTotal > 0 && (
          <div class={`${this.componentName}__list-pagination`}>
            <ElPagination props={this.paginationProps} onChange={this.handlePaginationChange} />
          </div>
        )}
        {this.renderFooter()}
      </div>
