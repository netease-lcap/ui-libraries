...
import { ElNode, KeysType } from '../common';

export interface ElTransferProps<T extends DataOption = DataOption> {
  /**
   * 用于控制复选框属性
   */
   ...
   * @default ''
   */
  empty?: EmptyType | Array<EmptyType> | ElNode;
  /**
   * 穿梭框底部内容
   */
  footer?: Array<string | ElNode> | ElNode<{ type: TransferListType }>;
  /**
   ...
   * @default []
   */
  title?: Array<TitleType> | ElNode<{ type: TransferListType }>;
  /**
   * 自定义渲染节点
   */
  transferItem?: ElNode<TransferItem<T>>;
  /**
   * 传入 Tree 组件定义树形结构
   */
  tree?: (tree: TreeProps) => ElNode;
  /**
   * 目标数据列表数据
   * @default []
   */
...
export type EmptyType = string | ElNode;

export type SearchOption = boolean | InputProps;

export type TitleType = string | ElNode;
