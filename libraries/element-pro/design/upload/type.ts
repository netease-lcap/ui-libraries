...
import { PlainObject, ElNode, UploadDisplayDragEvents } from '../common';

export interface ElUploadProps<T extends UploadFile = UploadFile> {
  /**
  ...
  cancelUploadButton?:
    | null
    | ButtonProps
    | ElNode<{ disabled: boolean; cancelUploadText: string; cancelUpload: (ctx: { e: MouseEvent }) => void }>;
  /**
   ...
   */
  dragContent?: ElNode | ElNode<TriggerContext>;
  /**
   ...
   */
  fileListDisplay?: ElNode<{ files: UploadFile[]; dragEvents?: UploadDisplayDragEvents }>;
  /**
  ...
  uploadButton?:
    | null
    | ButtonProps
    | ElNode<{ disabled: boolean; uploading: boolean; uploadFiles: () => void; uploadText: string }>;
  /**
   * 是否允许粘贴上传剪贴板中的文件
   * @default true
   */
