...
export default function useTreeSelect(props: ElTreeSelectProps, context: SetupContext) {
  ...
  function getMultipleNodeInfo(): TreeOptionData[] {
    ...
    if (treeRef.value) {
      return list.map((item) => {
        ...
        return (
          treeRef.value.getItem(finalValue)?.data || obj
        );
      });
    }
  ...
  // label could be used as ElNode, then use text to filter
