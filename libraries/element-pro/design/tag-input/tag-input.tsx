...
import { CloseCircleFilledIcon as ElCloseCircleFilledIcon } from '@element-ui-icons';
import ElInput, { InputProps } from '../input';
...
export default defineComponent({
  ...
  setup(props: ElTagInputProps, context) {
    ...
    const { CloseCircleFilledIcon } = useGlobalIcon({ CloseCircleFilledIcon: ElCloseCircleFilledIcon });
    ...
    const onInputEnter = (value: string, context: { e: KeyboardEvent }) => {
      ...
      setElInputValue('', { e: context.e, trigger: 'enter' });
    ...
    const onClearClick = (ctx: { e: MouseEvent }) => {
      clearAll(ctx);
      setElInputValue('', { e: ctx.e, trigger: 'clear' });
      props.onClear?.(ctx);
  ...
  render(h) {
    ...
    const suffixIconNode = this.showClearIcon ? (
      ...
      renderElNodeJSX(this, 'suffixIcon')
    );
    ...
    const prefixIconNode = renderElNodeJSX(this, 'prefixIcon');
    // 自定义 Tag 节点
    const displayNode = renderElNodeJSX(this, 'valueDisplay', {
      params: {
        value: this.tagValue,
    ...
    // 左侧文本
    const label = renderElNodeJSX(this, 'label', { silent: true });
      ...
      <ElInput
        ...
        on={{
          change: (val: string, context?: { e?: InputEvent | MouseEvent }) => {
            this.setElInputValue(val, { ...context, trigger: 'input' });
          },
          ...
          blur: (inputValue: string, context: { e: MouseEvent }) => {
            this.setElInputValue('', { e: context.e, trigger: 'blur' });
