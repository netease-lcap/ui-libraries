...
type RenderTsTypes = VNode | VNodeChildren | ElNode | RenderTsTypesSimple | RenderTsTypesObject;
// 组件render属性的js类型
...
// 通过template的方式渲染ElNode
export const RenderElNodeTemplate = Vue.extend({
...
/**
 * 通过JSX的方式渲染 ElNode，props 和 插槽同时处理，也能处理默认值为 true 则渲染默认节点的情况
 * @param vm 组件实例
 * @param name 插槽和属性名称
 * @param options 值可能为默认渲染节点，也可能是默认渲染节点和参数的集合
 * @example renderElNodeJSX(this, 'closeBtn')  优先级 props function 大于 插槽
 * @example renderElNodeJSX(this, 'closeBtn', <el-icon-close />)。 当属性值为 true 时则渲染 <el-icon-close />
 * @example renderElNodeJSX(this, 'closeBtn', { defaultNode: <el-icon-close />, params })。 params 为渲染节点时所需的参数
 */
export const renderElNodeJSX = (vm: VmType, name: string, options?: ScopedSlotReturnValue | JSXRenderContext) => {
  // 是否静默日志
...
/**
 * 通过JSX的方式渲染 ElNode，props 和 插槽同时处理。与 renderElNodeJSX 区别在于 属性值为 undefined 时会渲染默认节点
 * @param vm 组件实例
 * @param name 插槽和属性名称
 * @example renderElNodeJSXDefault(this, 'closeBtn')
 * @example renderElNodeJSXDefault(this, 'closeBtn', <el-icon-close />)。this.closeBtn 为空时，则兜底渲染 <el-icon-close />
 * @example renderElNodeJSXDefault(this, 'closeBtn', { defaultNode: <el-icon-close />, params }) 。params 为渲染节点时所需的参数
 */
export const renderElNodeJSXDefault = (vm: VmType, name: string, options?: ScopedSlotReturnValue | JSXRenderContext) => {
  const defaultNode = typeof options === 'object' && 'defaultNode' in options ? options.defaultNode : options;
  return renderElNodeJSX(vm, name, options) || defaultNode;
};
...
export const renderContent = (vm: VmType, name1: string, name2: string, options?: VNode | JSXRenderContext) => {
  ...
  const node1 = renderElNodeJSX(vm, name1, toParams);
  const node2 = renderElNodeJSX(vm, name2, toParams);
