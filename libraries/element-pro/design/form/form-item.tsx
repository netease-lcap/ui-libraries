...
import { ClassName, ElNodeReturnValue, Styles } from '../common';
...
// type Result = ValidateResult<ElFormProps['data']>;
...
export default mixins(getConfigReceiverMixins<FormItemConstructor, FormConfig>('form'), getGlobalIconMixins()).extend({
  ...
  methods: {
    ...
    async analysisValidateResult(trigger: ValidateTriggerType) {
      ...
      // 过滤不需要校验的规则
      result.rules = trigger === 'all' ? this.innerRules : this.innerRules.filter((item) => item.trigger === 'all' || (item.trigger || 'change') === trigger);
    ...
    getLabelContent(): ElNodeReturnValue {
    ...
    getDefaultIcon(): ElNodeReturnValue {
    ...
    getIcon(
      statusIcon: ElFormProps['statusIcon'] | ElFormItemProps['statusIcon'],
      slotStatusIcon: NormalizedScopedSlot,
      props?: ElFormItemProps,
    ): ElNodeReturnValue {
      const resultIcon = (otherContent?: ElNodeReturnValue) => (
    ...
    getSuffixIcon(): ElNodeReturnValue {
      const parent = this.form;
      ...
      let resultIcon: ElNodeReturnValue = this.getIcon(statusIcon, slotStatusIcon);
  ...
  render(): VNode {
    const helpNode = renderElNodeJSX(this, 'help');
    return (
