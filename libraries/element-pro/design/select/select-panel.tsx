...
import {
  ElOptionProps, SelectOptionGroup, ElSelectProps, SelectOption,
} from './type';
...
export interface OptionsType extends ElOptionProps {
  $index?: number;
}
        ...
        <ul class={`${this.componentName}__list`}>
          {options.map(
            (
              ...
              if (item.group) {
                return (
                  <el-option-group label={item.group} divider={item.divider}>
    ...
    renderPanelContent(innerStyle = {}) {
      ...
      return (
        <div
          ...
          style={innerStyle}
        >
          {this.renderElNode('panelTopContent')}
          ...
          {this.renderElNode('panelBottomContent')}
        </div>
      );
    },
  },
