...
import { PlainObject, ElNode, SizeEnum, KeysType, InfinityScroll } from '../common';

export interface ElSelectProps<T extends SelectOption = SelectOption> {
  /**
   * 宽度随内容自适应
   * @default false
   */
   ...
   * 面板内的底部内容
   */
  panelBottomContent?: string | ElNode;
  /**
   * 面板内的顶部内容
   */
  panelTopContent?: string | ElNode;
  /**
   * 占位符
   */
  placeholder?: string;
   ...
   * 自定义选中项呈现的内容
   */
  valueDisplay?: string | ElNode<{ value: SelectValue; onClose: (index: number) => void; displayValue?: SelectValue }>;
  /**
...
export interface ElOptionGroupProps {
  /**
   * 是否显示分隔线
   * @default true
   */
...
export type SelectOption = ElOptionProps | SelectOptionGroup | PlainObject;

export interface SelectOptionGroup extends ElOptionGroupProps {
  group: string;
  children: Array<ElOptionProps>;
}
