...
import {
  ElSelectProps, SelectValue, ElOptionProps, SelectValueChangeTrigger, SelectOption
} from './type';
...
export default defineComponent({
  ...
  setup(props: ElSelectProps, ctx) {
    ...
    const [tInputValue, setElInputValue] = useDefaultValue(
      inputValue,
    ...
    const handleCheckAllClick = (e: MouseEvent | KeyboardEvent) => {
      ...
      !reserveKeyword?.value && setElInputValue('');
    };
    ...
    const handleCreate = () => {
      ...
      multiple.value && setElInputValue('');
    ...
    const handleElInputValueChange = (val: string, context: SelectInputValueChangeContext) => {
    ...
    const keydownEvent = (e: KeyboardEvent) => {
      const displayOptions: (ElOptionProps & { isCreated?: boolean })[] = flattenOptions(
      ...
      const arrowDownOption = () => {
        ...
        while (hoverIndex.value < displayOptionsLength) {
          if (!(displayOptions[hoverIndex.value] as ElOptionProps)?.disabled) {
            break;
          }
      ...
      switch (e.code) {
        ...
        case 'Enter':
          ...
          // enter 选中逻辑
          if (multiple.value && (displayOptions[hoverIndex.value] as ElOptionProps).checkAll) {
            handleCheckAllClick(e);
          } else {
            const optionValue = (displayOptions[hoverIndex.value] as ElOptionProps)?.value;
    ...
    watch(innerPopupVisible, (value) => {
      if (value) {
        ...
        tInputValue.value && setElInputValue('');
      }
    });
    provide('tSelect', {
      size,
      ...
      handlerInputChange: setElInputValue,
  ...
  methods: {
    renderSuffixIcon() {
      const suffixIcon = this.renderElNode('suffixIcon');
      ...
      if (isLoading) {
        return (
          <el-loading class={[`${this.componentName}__right-icon`, `${this.componentName}__active-icon`]} size="small" />
        );
      }
      return showArrow ? (
        <fake-arrow
    ...
    renderLabel() {
      const label = this.renderElNode('label');
      const prefixIcon = this.renderElNode('prefixIcon');
      if (label && prefixIcon) {
        return (
          <div>
            {label}
        ...
        <SelectInput
          ...
          {...{
            props: {
              ...
              valueDisplay: () => renderElNode('valueDisplay', { params: this.valueDisplayParams }),
              ...
              collapsedItems: () => renderElNode('collapsedItems', { params: this.collapsedItemsParams }),
            ...
            on: {
              ...
              'input-change': this.handleElInputValueChange,
