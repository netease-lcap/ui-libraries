...
export default function useTreeStore(state: TypeTreeState) {
  ...
  const tActived = state.vmActived[0] as TypeRef<TypeElNodeValue[]>;
  // tExpanded 就是 refProps.expanded
  const tExpanded = state.vmExpanded[0] as TypeRef<TypeElNodeValue[]>;
  ...
  let prevExpanded: null | TypeElNodeValue[] = null;
  ...
  const expandFilterPath = () => {
    ...
    if (props.filter) {
      ...
      // 展开搜索命中节点的路径节点
      const pathValues: TypeElNodeValue[] = [];
