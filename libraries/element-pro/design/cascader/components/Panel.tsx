...
export default defineComponent({
  ...
  setup(props, { emit }) {
    const renderElNodeJSXDefault = useElNodeDefault();
  ...
  render() {
    const {
      global, COMPONENT_NAME, handleExpand, renderElNodeJSXDefault, cascaderContext, panels, emit,
    } = this;
    ...
    const renderEmpty = () => {
      ...
      return renderElNodeJSXDefault('empty', <div class={`${COMPONENT_NAME}__panel--empty`}>{global.empty}</div>);
    };
    ...
    let content;
    if (this.loading) {
      content = renderElNodeJSXDefault(
        'loadingText',
