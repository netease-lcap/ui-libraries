...
import {
  CascaderValue, ElSelectInputProps, ElCascaderProps,
} from './interface';
...
export default defineComponent({
  ...
  render() {
    ...
    const renderValueDisplay = () => renderElNodeJSX(this, 'valueDisplay', {
      params: valueDisplayParams,
    });

    const renderSuffixIcon = () => {
      const suffixIcon = renderElNodeJSX(this, 'suffixIcon');
      ...
      <SelectInput
        ...
        {...{
          props: {
            ...
            popupProps: {
              ...(this.popupProps as ElCascaderProps['popupProps']),
              overlayInnerStyle: panels.length && !this.loading ? { width: 'auto' } : '',
              overlayClassName: [
                overlayClassName,
                (this.popupProps as ElCascaderProps['popupProps'])?.overlayClassName,
              ],
            },
            inputProps: { size: this.size, ...(this.inputProps as ElCascaderProps['inputProps']) },
            tagInputProps: { size: this.size, ...(this.tagInputProps as ElCascaderProps['tagInputProps']) },
            tagProps: { ...(this.tagProps as ElCascaderProps['tagProps']) },
            onInputChange: (value: string, ctx: SelectInputValueChangeContext) => {
              if (!this.isFilterable) return;
              setInputVal(`${value}`);
              (this?.selectInputProps as ElSelectInputProps)?.onInputChange?.(value, ctx);
            },
            onTagChange: (val: TagInputValue, ctx: SelectInputChangeContext) => {
              ...
              (this.selectInputProps as ElSelectInputProps)?.onTagChange?.(val, ctx);
            },
            onPopupVisibleChange: (val: boolean, context: PopupVisibleChangeContext) => {
              if (this.disabled) return;
              setVisible(val, context);
              (this.selectInputProps as ElSelectInputProps)?.onPopupVisibleChange?.(val, context);
            },
            onBlur: (val: CascaderValue, context: SelectInputFocusContext) => {
              ...
              this.onBlur?.(ctx);
              emit('blur', ctx);
              (this.selectInputProps as ElSelectInputProps)?.onBlur?.(val, context);
            },
            onFocus: (val: CascaderValue, context: SelectInputFocusContext) => {
              const ctx = { value: cascaderContext.value, e: context.e };
              this.onFocus?.(ctx);
              emit('focus', ctx);
              (this.selectInputProps as ElSelectInputProps)?.onFocus?.(val, context);
            },
            onClear: (context: { e: MouseEvent }) => {
              closeIconClickEffect(cascaderContext);
              (this.selectInputProps as ElSelectInputProps)?.onClear?.(context);
            },
            ...omit(this.selectInputProps as ElSelectInputProps, [
              'onTagChange',
              'onInputChange',
