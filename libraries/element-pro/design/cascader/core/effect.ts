...
import {
  TreeNode, CascaderContextType, ElCascaderProps, TreeNodeValue, TreeNodeModel,
} from '../interface';
...
export function expendClickEffect(
  propsTrigger: ElCascaderProps['trigger'],
  trigger: ElCascaderProps['trigger'],
  node: TreeNode,
...
export function valueChangeEffect(node: TreeNode, cascaderContext: CascaderContextType) {
  ...
  if (isNumber(max) && max < 0) {
    console.warn('ElementUI Warn:', 'max should > 0');
  }
...
export function handleRemoveTagEffect(
  cascaderContext: CascaderContextType,
  index: number,
  onRemove: ElCascaderProps['onRemove'],
) {
  const {
