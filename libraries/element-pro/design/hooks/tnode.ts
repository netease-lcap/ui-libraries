...
/**
 * 通过 JSX 的方式渲染 ElNode，props 和 插槽同时处理，也能处理默认值为 true 则渲染默认节点的情况
 * 优先级：Props 大于插槽
 * 如果 props 值为 true ，则使用插槽渲染。如果也没有插槽的情况下，则使用 defaultNode 渲染
 * @example const renderElNodeJSX = useElNodeJSX()
 * @return () => {}
 * @param name 插槽和属性名称
 * @param options 值可能为默认渲染节点，也可能是默认渲染节点和参数的集合
 * @example renderElNodeJSX('closeBtn')  优先级 props function 大于 插槽
 * @example renderElNodeJSX('closeBtn', <close-icon />)。 当属性值为 true 时则渲染 <close-icon />
 * @example renderElNodeJSX('closeBtn', { defaultNode: <close-icon />, params })。 params 为渲染节点时所需的参数
 */
export const useElNodeJSX = () => {
...
/**
 * 在setup中，通过JSX的方式 ElNode，props 和 插槽同时处理。与 renderElNodeJSX 区别在于属性值为 undefined 时会渲染默认节点
 * @example const renderElNodeJSXDefault = useElNodeDefault()
 * @return () => {}
 * @param name 插槽和属性名称
 * @example renderElNodeJSXDefault('closeBtn')
 * @example renderElNodeJSXDefault('closeBtn', <close-icon />) closeBtn 为空时，则兜底渲染 <close-icon />
 * @example renderElNodeJSXDefault('closeBtn', { defaultNode: <close-icon />, params }) 。params 为渲染节点时所需的参数
 */
export const useElNodeDefault = () => {
  const renderElNodeJSX = useElNodeJSX();
  return function (name: string, options?: VNode | JSXRenderContext) {
    const defaultNode = getDefaultNode(options);
    return renderElNodeJSX(name, options) || defaultNode;
  };
};

/**
 * 在setup中，用于处理相同名称的 ElNode 渲染
...
export const useContent = () => {
  ...
  return function (name1: string, name2: string, options?: VNode | JSXRenderContext) {
    ...
    const node1 = renderElNodeJSX(name1, toParams);
    const node2 = renderElNodeJSX(name2, toParams);
