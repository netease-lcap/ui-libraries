...
import type { ElNode, ClassName } from '../common';
import type { ElPopupProps } from './type';
...
const Overlay = mixins(classPrefixMixins).extend({
  ...
  render(h): VNode {
    const content = renderElNodeJSX(this, 'content');
...
export type PluginMethod = (triggerEl: string | HTMLElement, content: ElNode, popupProps?: ElPopupProps) => Instance;
...
export const createPopupPlugin: PluginMethod = (trigger, content, popupProps) => {
  ...
  popperInstance = createPopper(triggerEl, overlayInstance, {
    placement: getPopperPlacement(popupProps?.placement || ('top' as ElPopupProps['placement'])),
