...
namespace nasl.ui {
  ...
  export class ElCascaderPro extends ViewComponent {
    constructor(options?: Partial<ElCascaderProOptions>) {
      super();
    }
  }

  export class ElCascaderProOptions extends ViewComponentOptions {
    @Prop({
      group: '主要属性',
      title: 'Autofocus',
      description: '自动聚焦',
    ...
    @Prop({
      ...
      setter: {
        ...
        options: [
          { title: 'large' },
          { title: 'medium' },
          { title: 'small。TS 类型：`SizeEnum`。[通用类型定义](https:' },
          { title: '' },
          { title: 'github.com' },
          { title: 'Tencent' },
          { title: 'tdesign-vue' },
    ...
    size:
      | 'large'
      | 'medium'
      | 'small。TS 类型：`SizeEnum`。[通用类型定义](https:'
      | ''
      | 'github.com'
      | 'Tencent'
      | 'tdesign-vue'
