  ...
  if (script) {
    ...
    if (
      ...
      if (script.lang === 'ts') {
        ...
        if (!map.file && script.map?.file) map.file = script.map.file;

        if (!map.sourcesContent && script.map?.sourcesContent) map.sourcesContent = script.map.sourcesContent;
      }
    } else {
      ...
      scriptCode = `import ${scriptVar} from ${request}\n` + `export * from ${request}`; // support named exports
    }
  }
  return {
    scriptCode,
...
async function genTemplateRequest(
  ...
  if (!template) return { code: 'let __vue2_render, __vue2_staticRenderFns' };
...
function normalizeDevPath(root: string, id: string) {
  if (id.startsWith(`${root}/`)) return id.slice(root.length);
  if (fs.existsSync(cleanUrl(id))) return FS_PREFIX + id;
