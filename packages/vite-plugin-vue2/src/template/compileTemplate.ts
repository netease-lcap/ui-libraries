...
export function compileTemplate(
  ...
  const preprocessor = preprocessLang && consolidate[preprocessLang as keyof typeof consolidate];
  if (preprocessor) {
    return actuallyCompile(
      { ...options, source: preprocess(options, preprocessor) },
    );
  }
  if (preprocessLang) {
    return {
...
function actuallyCompile(
  ...
  let finalCompilerOptions: any = compilerOptions;
  ...
  const {
    ast, render, staticRenderFns, tips, errors,
  } = compile(
    source,
    finalCompilerOptions,
  );
  ...
  const finalTranspileOptions = {
    ...transpileOptions,
    transforms: { ...transpileOptions.transforms, stripWithFunctional: isFunctional },
  };
